function PhasePlot2pop(C, x0_mat, y0_mat, tspan, titleStr)
% PHASEPLOT2POP  Phase portrait of the two-population replicator dynamics.
%
%   PhasePlot2pop(C, x0_mat, y0_mat, tspan, titleStr)
%
%   Plots TWO side-by-side ternary simplices:
%     LEFT  = Population 1 trajectories  (x-simplex)
%     RIGHT = Population 2 trajectories  (y-simplex)
%
...
for i = 1:K
    [~, Xi, Yi] = xRepDyn2pop(C, x0_mat(i,:)', y0_mat(i,:)', tspan);

    % Pop 1 (left panel)
    subplot(1,2,1); hold on;
    [tx, ty] = ternary2cart(Xi);
...
% Mark pure vertices in both panels
V2d = [0,0;1,0;0.5,sqrt(3)/2];
for pop = 1:2
