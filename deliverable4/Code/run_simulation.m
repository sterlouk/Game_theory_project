% Script to run the 5-strategy replicator dynamics simulation

% --- Parameters ---
M = 4;
T = 50;
p_values = [0.75, 0.60];
tspan = [0 150]; % Increased time span for convergence

% --- Initial Conditions ---
% A set of initial conditions to explore the 5-strategy simplex
x0_mat = [
...
for i = 1:length(p_values)
    ...
    % Save the figure
    fig_filename = sprintf('phase_portrait_p_%.2f.png', p);
    saveas(fig_handle, fig_filename);
    fprintf('Saved figure: %s\n', fig_filename);
    
    % Find and display rest points
    fprintf('Rest points for p = %.2f:\n', p);
    % Note: This requires a separate function to find rest points,
    % which is not implemented yet. For now, we observe from the plot.
